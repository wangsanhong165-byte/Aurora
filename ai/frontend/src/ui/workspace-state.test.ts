...
test('selecting the active drawer closes it and selecting another replaces it', () => {
  const initial: DrawerState = { active: 'chat', width: 380 }

  assert.deepEqual(reduceDrawerState(initial, { type: 'select', section: 'chat' }), {
    active: null,
    width: 380,
  })
