...
> implements ICubismClippingManager {
  ...
  public calcClippedDrawableTotalBounds(
    ...
    // 被クリッピングマスク（マスクされる描画オブジェクト）の全体の矩形
    const clippedBounds = createEmptyClippingBounds();
    ...
    for (
      ...
      // 全体の矩形に反映
      includeClippingBounds(clippedBounds, minX, minY, maxX, maxY);

      if (clippedBounds.minX == Number.MAX_VALUE) {
        clippingContext._allClippedDrawRect.x = 0.0;
        clippingContext._allClippedDrawRect.y = 0.0;
        clippingContext._allClippedDrawRect.width = 0.0;
        clippingContext._allClippedDrawRect.height = 0.0;
        ...
        clippingContext._isUsing = true;
        const w: number = clippedBounds.maxX - clippedBounds.minX;
        const h: number = clippedBounds.maxY - clippedBounds.minY;
        clippingContext._allClippedDrawRect.x = clippedBounds.minX;
        clippingContext._allClippedDrawRect.y = clippedBounds.minY;
        clippingContext._allClippedDrawRect.width = w;
        clippingContext._allClippedDrawRect.height = h;
      }
    }
